...
class Borrow(Container):
    ...
    def go_to_confirm(self):
        ...
        if "" in listOfEntrys: #checks missing
            ...
            #AN Entry
            self.input_AN = Label(self.container, text=self.AN_entry.get(), bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            ...
            self.input_BT = Label(self.container, text=data_BT, bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE)) 
            self.input_BT.place(relx=0.6, rely=0.35, anchor="center")
            #Borrow date Entry
            self.input_BD = Label(self.container, text=date.today(), bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE)) 
            self.input_BD.place(relx=0.6, rely=0.4, anchor="center")
            #Membership ID Entry
            self.input_ID = Label(self.container, text=self.ID_entry.get(), bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            ...
            self.input_name = Label(self.container, text=data_name, bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.input_name.place(relx=0.6, rely=0.5, anchor="center")
            #Due Date Entry
            self.input_DD = Label(self.container, text=date.today() + timedelta(days=14), bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.input_DD.place(relx=0.6, rely=0.55, anchor="center")
            #Labels
            self.AN_label = Label(self.container, text = "Accession Number:", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.AN_label.place(relx=0.4, rely=0.3, anchor="center")
            self.BT_label = Label(self.container, text = "Book Title:", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.BT_label.place(relx=0.4, rely=0.35, anchor="center")
            self.BD_label = Label(self.container, text = "Borrow Date:", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.BD_label.place(relx=0.4, rely=0.4, anchor="center")
            self.ID_label = Label(self.container, text = "Membership ID:", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.ID_label.place(relx=0.4, rely=0.45, anchor="center")
            self.name_label = Label(self.container, text = "Member Name:", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.name_label.place(relx=0.4, rely=0.5, anchor="center")
            self.DD_label = Label(self.container, text = "Due Date:", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            ...
            self.confirmLoanButton = Button(self.container, text="Confirm Loan", padx=20, pady=20,\
                command=self.go_to_error, bg="#27c0ab",borderwidth=5, highlightthickness=4, highlightbackground="#ecb606", relief="raised", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.confirmLoanButton.place(relx=0.4, rely=0.70, anchor="center")
             #Back to borrow function button
            self.backBorrowButton = Button(self.container, text="Back to Borrow Function", padx=20, pady=20,\
                bg="#27c0ab",borderwidth=5, highlightthickness=4, highlightbackground="#ecb606", relief="raised", command=self.close_confirmPage, font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.backBorrowButton.place(relx=0.6, rely=0.70, anchor="center")
            
    def failed(self):   
        self.ErrorPop = Label(self.container, text='Error!\n\n Duplicate, Missing or Incomplete fields.', fg='yellow', bg='#FF0000',\
        ...
        self.backBorrowButton = Button(self.container, text="Back to Borrow Function", padx=20, pady=20,\
            bg="#27c0ab",borderwidth=5, highlightthickness=4, highlightbackground="#ecb606", relief="raised",\
            command=lambda:self.close(self.ErrorPop, self.backBorrowButton))
        self.backBorrowButton.config(font=(FONT, FONT_SIZE, STYLE))
        self.backBorrowButton.place(relx=0.5, rely=0.70, anchor="center")
...
class Return(Container):
    ...
    def go_to_confirm(self):
        ...
        if "" in listOfEntrys: #checks missing
            ...
            #Prompt
            self.popupPromptLabel = Label(self.container, text="Confirm Return Details To Be Correct", 
            width = 40, height=15, anchor='n', bg="#9ddd58", fg="#000000", font=(FONT,FONT_SIZE, STYLE))
            self.popupPromptLabel.place(relx=0.5, rely=0.5, anchor='center')
            #AN Entry
            self.input_AN = Label(self.container, text=self.AN_entry.get(), bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.input_AN.place(relx=0.6, rely=0.3, anchor="center")
            ...
            self.input_ID = Label(self.container, text=data_ID, bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE)) 
            ...
            #RD Label
            self.input_RD = Label(self.container, text=self.RD_entry.get(), bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            ...
            self.input_fine = Label(self.container, text="$" + str(fine_amt), bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            ...
            self.RD_label = Label(self.container, text = "Return Date", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.RD_label.place(relx=0.4, rely=0.5, anchor="center")
            self.fine_label = Label(self.container, text = "Fine:", bg="#9ddd58", fg="#000000", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            ...
            command=self.go_to_error, bg="#27c0ab",borderwidth=5, highlightthickness=4, highlightbackground="#ecb606", relief="raised", font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.confirmReturnButton.place(relx=0.4, rely=0.70, anchor="center")
             #Back to borrow function button
            self.backBorrowButton = Button(self.container, text="Back to Borrow Function", padx=20, pady=20, 
             bg="#27c0ab",borderwidth=5, highlightthickness=4, highlightbackground="#ecb606", relief="raised", command=self.close_confirmPage, font=(FONT,NOTIF_FONT_SIZE, STYLE))
            self.backBorrowButton.place(relx=0.6, rely=0.70, anchor="center")
    ...
    def failed(self):   
        ...
        fg='yellow', bg='#FF0000',
        relief='raised', width=50, height=15)
        self.ErrorPop.config(font=(FONT, FONT_SIZE, STYLE))
        self.ErrorPop.place(relx=0.5, rely=0.5, anchor="center")
        #Back to return function button
        self.backReturnButton = Button(self.container, text="Back to Return Function", padx=20, pady=20, 
            bg="#27c0ab",borderwidth=5, highlightthickness=4, highlightbackground="#ecb606", relief="raised", command=self.close_incompleteError, font=(FONT,NOTIF_FONT_SIZE, STYLE))
        self.backReturnButton.place(relx=0.5, rely=0.7, anchor="center")       
    ...
    def go_to_fineError(self):
        ...
        fg='yellow', bg='#FF0000',
        relief='raised', width=40, height=15, font=(FONT,FONT_SIZE, STYLE))
        #Find memberid
        sql_statement = "SELECT BorrowerID FROM loan WHERE BorrowedBookAccession = '{}'".format(self.AN_entry.get())
        data_ID = self.cursor.execute(sql_statement).fetchall()[0][0]
        sql_statement = "SELECT BorrowDate FROM loan WHERE BorrowedBookAccession = '{}'".format(self.AN_entry.get())
